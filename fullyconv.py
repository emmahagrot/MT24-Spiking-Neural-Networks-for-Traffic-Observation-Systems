...
def save_data(model, train_loss, validation_loss, tau, today, epoch):
    file_name = f'{today}_Tau_{tau}_fullyconv'
...
for tau_mem in tau_list:   
    ...
    class SNN(nn.Module):
        def __init__(self):
            ...
            self.conv1 = nn.Conv2d(1, 8, kernel_size=13, stride=2, padding=0)
            self.bn1 = nn.BatchNorm2d(8)
            self.lif1 = LIFCell(p=LIFParameters(tau_mem_inv=tau_mem))
            
            self.conv2 = nn.Conv2d(8, 8, kernel_size=11, stride=1, padding=0) 
            self.bn2 = nn.BatchNorm2d(8)
            self.lif2 = LIFCell(p=LIFParameters(tau_mem_inv=tau_mem))
            
            self.conv3 = nn.Conv2d(8, 8, kernel_size=9, stride=1, padding=0)  
            self.bn3 = nn.BatchNorm2d(8)
            self.lif3 = LIFCell(p=LIFParameters(tau_mem_inv=tau_mem))
            
            self.conv4 = nn.Conv2d(8, 8, kernel_size=7, stride=1, padding=0) 
            self.bn4 = nn.BatchNorm2d(8)
            self.lif4 = LIFCell(p=LIFParameters(tau_mem_inv=tau_mem))
            
            self.conv5 = nn.Conv2d(8, 1, kernel_size=7, stride=1, padding=0)  
            self.bn5 = nn.BatchNorm2d(1)
            self.lif5 = LICell(p=LIFParameters(tau_mem_inv=tau_mem))
        ...
        def forward(self, x, mem_states):
            ...
            spike_rec = []
            current_rec = []
            mem1, mem2, mem3, mem4, mem5 = mem_states
            ...
            return spk5, (mem1, mem2, mem3, mem4, mem5)
            ...
            for data_nr in range(26):
                ...
                if data is not None:
                    ...
                    for i, (frames, targets) in enumerate(validation_loader):
                        ...
                        for step in range(sequence_length):
                            input_frame = frames[:, step].unsqueeze(1)
                            output, current, spikes, mem_states = model(input_frame, mem_states)
                            final_output = output.view(16, 64, 64)
